...
def runVideo(download_throughput, id, driverPath):
	...
	prefix = "outputs/YT_" + get_exp_config(id)['ytId'] + str(download_throughput)
	...
	try:
		...
		# options.add_argument(argument='--load-extension=D:/SBU/extension/video_collection/extension')
		print(time.time(), ' start chrome')
		# browser = webdriver.Chrome("D:\SBU\Fall20\CSE 534 FCN\chromedriver") #download_throughput
		browser = webdriver.Chrome(driverPath)  # download_throughput

		browser.set_network_conditions(
			offline=False,
			latency=5,  # additional latency (ms)
			download_throughput=download_throughput * 1024,  # maximal throughput
			upload_throughput=500 * 1024)  # maximal throughput

		time.sleep(10)

		# read in js
		jsFile = open('pluginAsJSFormated.js', 'r')
		js = jsFile.read()
		jsFile.close

		# open webpage
		print(time.time(), ' start video ', get_exp_config(id)['ytId'])
		browser.get(url)
		browser.get_screenshot_as_file('./screenshot0.png')

		# inject js
		browser.execute_script(js)
		duration = get_exp_config(id)['duration']
		time.sleep(duration)
		...
		with open(prefix + '_buffer.txt', 'wb') as f:
			f.write(out.encode("UTF-8"))
		with open(prefix + '_events.txt', 'wb') as f:
	...
	bitrates = get_exp_config(id)['bitrates'].split(",")
	with open(prefix + '_outStream.txt', 'wb') as f:
...
def getOutput(prefix, bitrates):
	out = "calculate bitrates \n" + calculateBitrate(prefix, bitrates) + "calculate Buffer \n," + calculateBuffer(
		prefix) + "calculate stallling \n," + calculateStallings(prefix)
	return out
...
def getEvents(prefix):
	timestamps = []
	qualities = []
	with open(prefix + "_events.txt", "r") as filestream:
...
def getBuffer(prefix):
	...
	avPlaytime = []
	isFirstLine = True
	with open(prefix + "_buffer.txt", "r") as filestream:
		for line in filestream:
			currentline = line.split("#")
			# end of video
...
def calculateBitrate(prefix, bitrates):
	...
	return str(avgBitrate) + "," + str(maxBitrate) + "," + str(minBitrate) + "," + str(q25) + "," + str(
		q50) + "," + str(q75) + "," + str(q90)
...
def calculateBuffer(prefix):
	...
	return str(avgBuffer) + "," + str(maxBuffer) + "," + str(minBuffer) + "," + str(q25) + "," + str(q50) + "," + str(
		q75) + "," + str(q90)
...
def calculateStallings(prefix):
	...
	return str(numOfStallings) + "," + str(avgStalling) + "," + str(maxStalling) + "," + str(minStalling) + "," + str(
		q25) + "," + str(q50) + "," + str(q75) + "," + str(q90)
...
def main():
	...
	# Start Measurements
	for id in yt_ids:
		for val in download_throughputs:
			runVideo(val, id, driverPath)
